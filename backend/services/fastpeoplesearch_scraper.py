"""
FastPeopleSearch.com scraper — JSON-LD structured data for phone + email.

Similar to TruePeopleSearch with heavy Cloudflare protection.
- Phone numbers + email addresses in JSON-LD Person schema
- Family connections, aliases, addresses
- Rate-limited at 2s. Cached 7 days.
"""
...
def _cache_key(name: str, city: str, state: str) -> str:
    raw = f"fps|{name.lower().strip()}|{city.lower().strip()}|{state.upper().strip()}"
...
def _scrape_sync(name: str, city: str, state: str) -> Optional[dict]:
    """Scrape FastPeopleSearch for person data."""
    ...
    try:
        ...
        if resp.status_code == 403:
            logger.debug(f"FastPeopleSearch 403 (Cloudflare) for {name}")
            return None
        if resp.status_code != 200:
            logger.debug(f"FastPeopleSearch {resp.status_code} for {name}")
            return None
        ...
        # Fallback: HTML pattern extraction
        phones = []
        emails = []
        ...
        # Detail card phone spans
        for el in soup.find_all(["span", "div"], class_=re.compile(r"phone|tel", re.I)):
        ...
        if phones or emails:
            return {
                ...
                "emails": emails[:5],
                "name": "",
                "source": "FastPeopleSearch-html",
            }

        return None

    except Exception as e:
        logger.debug(f"FastPeopleSearch error for {name}: {e}")
        return None


async def search(name: str, city: str = "", state: str = "") -> Optional[dict]:
    """Async name search on FastPeopleSearch."""
